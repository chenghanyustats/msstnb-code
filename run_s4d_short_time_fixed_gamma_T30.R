## ============================================================================
## run_s4d_short_time_fixed_gamma_T30.R
##
## Scenario 4D short-time-series stress test with gamma fixed.
##
## Purpose
...
##   data_s4d_short_time/S4D_SHORT_TIME_T30/data_repXX.rds
##
## Main outputs
...
##   run_s4d_short_time_fixed_gamma_T30(
##       root = ".",
##       reps = 1,
...
s4d_source_data_path <- function(rep_id,
                                 root_arg = ".",
                                 data_dir_arg = "data_s4d_short_time",
                                 data_scenario_id_arg = "S4D_SHORT_TIME_T30") {
    file.path(
        root_arg,
        data_dir_arg,
...
check_s4d_source_dataset <- function(data_file,
    ...
    if (exists("validate_s4d_short_time_data", mode = "function")) {
...
fit_s4d_short_time_fixed_gamma_one_rep <- function(rep_id,
                                                   ...
                                                   data_scenario_id = "S4D_SHORT_TIME_T30",
                                                   data_dir = "data_s4d_short_time",
                                                   output_dir = "output_s4d_short_time_fixed_gamma",
                                                   root = ".",
    ...
    cat(sprintf("=== Scenario 4D short-time fixed-gamma fit: rep %s ===\n", rr))
    ...
    fit$metadata$model <- "S4D short-time dynamic NB-ICAR with fixed gamma"
    ...
    fit$metadata <- c(
        ...
        list(
            ...
            fit_file_prefix = "fit_S4D_short_time_fixed_gamma_rep",
    ...
    if (isTRUE(save_result)) {
        ...
        fit_file <- file.path(out_dir, paste0("fit_S4D_short_time_fixed_gamma_rep", rr, ".rds"))
        csv_file <- file.path(out_dir, paste0("summary_S4D_short_time_fixed_gamma_rep", rr, ".csv"))
...
fit_s4d_short_time_fixed_gamma_batch <- function(reps = 1:10,
    ...
    summary_file <- file.path(out_dir, "summary_S4D_short_time_fixed_gamma_all_reps.csv")
...
print_s4d_fit_driver_summary <- function(source_manifest, fit_summary, fit_manifest) {
    cat("\n=== Scenario 4D short-time fixed-gamma fitting summary ===\n")
...
## ---- main driver ------------------------------------------------------------
run_s4d_short_time_fixed_gamma_T30 <- function(root = ".",
                                               reps = NULL,
                                               ...
                                               s4d_data_file = "s4d_short_time_series.R",
                                               fixed_gamma_value = 0.8,
                                               gamma_prior = c(1, 1)) {
    ...
    source_s4d_short_time_series(root = root, verbose = isTRUE(verbose))
    ...
    if (isTRUE(verbose)) {
        ...
        print(source_data_manifest[, c(
            ...
            "variance_to_mean", "q99_count", "kappa_cv", "beta0_star_ident",
            "lambda_raw_median", "lambda_raw_max", "log_lambda_sd", "delta_log_lambda_sd"
        ), drop = FALSE])
    }
    ...
    fit_files <- file.path(
        fit_dir_full,
        sprintf("fit_S4D_short_time_fixed_gamma_rep%02d.rds", as.integer(reps))
    )
    ...
    saveRDS(
        ...
        file.path(fit_dir_full, "run_info_S4D_short_time_fixed_gamma_T30.rds")
    )
    ...
    if (isTRUE(verbose)) {
        ...
        cat("\nScenario 4D short-time fixed-gamma T30 fitting finished successfully.\n")
    }
...
message("Loaded run_s4d_short_time_fixed_gamma_T30.R. Use run_s4d_short_time_fixed_gamma_T30(...) to fit S4D.")
