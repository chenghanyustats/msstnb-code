## ============================================================================
## s2_dynamic_fixed_gamma_v2.R
## Version: local-updater patch, avoids update_beta/update_phi/update_r helper conflicts.
##
...
.s2_target_envs_for_updates <- function() {
    envs <- list(.GlobalEnv)
    for (fn_nm in c("update_beta", "update_phi", "update_r", "update_kappa", "ffbs_lambda_all")) {
        if (exists(fn_nm, envir = .GlobalEnv, inherits = TRUE)) {
            fn <- get(fn_nm, envir = .GlobalEnv, inherits = TRUE)
            if (is.function(fn)) {
                envs[[length(envs) + 1L]] <- environment(fn)
            }
        }
    }
    ## Deduplicate environments by their printed address/name where possible.
    out <- list()
    for (ee in envs) {
        if (is.environment(ee) && !any(vapply(out, identical, logical(1), ee))) {
            out[[length(out) + 1L]] <- ee
        }
    }
    out
}

.s2_assign_update_helper <- function(name, value) {
    for (ee in .s2_target_envs_for_updates()) {
        assign(name, value, envir = ee)
    }
    invisible(TRUE)
}

.install_s2_check_same_dim_compat <- function() {
    ## Some project update files define private helpers with the same name but
    ## slightly different signatures.  Scenario 2 needs the regression helper
    ## signature used by update_beta/update_phi/update_r:
    ##     .check_same_dim(y, e, x1, x2, lambda_tilde, names = ...)
    ## This compatibility version is intentionally permissive and dimension-only.
    assign(
        ".check_same_dim",
        function(y, ..., names = NULL) {
            args <- list(...)
            if (is.null(dim(y))) {
                stop("The reference object must have dimensions.", call. = FALSE)
            }
            y_dim <- dim(y)

            ## Backward-compatible mode for calls like .check_same_dim(x, c(TT, n1)).
            if (length(args) == 1L && is.numeric(args[[1]]) && is.null(dim(args[[1]])) &&
                length(args[[1]]) == length(y_dim)) {
                if (!identical(as.integer(y_dim), as.integer(args[[1]]))) {
            ...
            if (is.null(names)) {
                names <- paste0("object_", seq_along(args))
            }
            if (length(names) < length(args)) {
                names <- c(names, paste0("object_", seq(from = length(names) + 1L, to = length(args))))
            }

            for (ii in seq_along(args)) {
                obj <- args[[ii]]
                nm <- names[[ii]]
                if (is.null(dim(obj)) || !identical(as.integer(dim(obj)), as.integer(y_dim))) {
                    stop(nm, " must have the same dimensions as the reference matrix.",
                         call. = FALSE)
                }
            }
            invisible(TRUE)
...
.install_s2_regression_check_compat <- function() {
    ## Override private input-check helpers used by the revised regression,
    ## ICAR, and dispersion updates.  Some project files reuse the same helper
    ## names with incompatible signatures.  These replacements are deliberately
    ## conservative: they validate dimensions and obvious scalar/vector lengths,
    ## then return invisibly without changing any model calculation.

    assign(
        ".check_beta_inputs",
        function(beta_current, y_coarse, e, x1, x2, lambda_tilde, phi, r, priors) {
            y_dim <- dim(y_coarse)
            if (is.null(y_dim) || length(y_dim) != 2L) {
                stop("y_coarse must be a matrix.", call. = FALSE)
            }
            for (nm in c("e", "x1", "x2", "lambda_tilde")) {
                obj <- get(nm, parent.frame())
                if (is.null(dim(obj)) || !identical(as.integer(dim(obj)), as.integer(y_dim))) {
                    stop(nm, " must have the same dimensions as y_coarse.", call. = FALSE)
                }
            }
            n1 <- y_dim[2]
            if (length(phi) != n1) {
            ...
            p <- length(priors$beta_mean %||% beta_current[-1])
    ...
    assign(
        ".check_phi_inputs",
        function(u_current, B, BHB, tau_phi, y_coarse, e, x1, x2,
            ...
            n1 <- y_dim[2]
            if (nrow(B) != n1) {
                stop("B must have nrow equal to ncol(y_coarse).", call. = FALSE)
            }
            if (length(u_current) != ncol(B)) {
                stop("u_current must have length ncol(B).", call. = FALSE)
            }
            if (!(length(r) == 1L || length(r) == n1)) {
                stop("r must be scalar or have length ncol(y_coarse).", call. = FALSE)
            }
    ...
    assign(
        ".check_r_inputs",
        function(r_current, y_coarse, e, x1, x2, lambda_tilde,
            ...
            n1 <- y_dim[2]
            if (!(length(r_current) == 1L || length(r_current) == n1)) {
                stop("r_current must be scalar or have length ncol(y_coarse).", call. = FALSE)
            }
            if (length(phi) != n1) {
                stop("phi must have length ncol(y_coarse).", call. = FALSE)
            }
...
source_s2_dynamic_fixed_gamma <- function(root = ".", verbose = TRUE) {
    ...
    ## Important: update_regression_revised.R and update_dispersion_revised.R
    ## are sourced so their shared utilities remain available, but Scenario 2
    ## does NOT call update_beta(), update_phi(), or update_r().  It uses local
    ## update_beta_s2(), update_phi_s2(), and update_r_s2() below, which avoids
    ## the private-helper signature conflict seen in the project-level updaters.
    if (isTRUE(verbose)) {
        message("Scenario 2 local-updater version loaded: project updater files sourced, but update_beta/update_phi/update_r are not called.")
    }
    ...
    needed <- c(
        ...
        "update_tau_phi", "update_kappa", "ffbs_lambda_all",
        "recenter"
    )
...
## -----------------------------------------------------------------------------
## Scenario 2 local MCMC updates
## -----------------------------------------------------------------------------
## These local updates intentionally avoid the private helper functions inside
## update_regression_revised.R, update_icar_revised.R, and
## update_dispersion_revised.R.  Some project files define helpers with the same
## names but incompatible signatures.  Using local Scenario 2 updates prevents
## those helper conflicts while preserving the same marginal NB likelihood used
## in Scenario 1, now conditional on the current lambda_tilde path.

.nb_loglik_s2 <- function(y, mu, r) {
...
.logpost_beta_s2 <- function(beta_full, y_coarse, e, x1, x2, lambda_tilde,
                             phi, r, priors) {
    beta0 <- beta_full[1]
    beta <- beta_full[-1]
    ll <- compute_s2_loglik_nb(
        y_coarse = y_coarse,
        e = e,
        x1 = x1,
        x2 = x2,
...
update_beta_s2 <- function(beta_current, y_coarse, e, x1, x2, lambda_tilde,
                           phi, priors, ess_base = NULL, r,
                           use_preconditioned = TRUE, max_steps = 200L) {
    y_dim <- dim(y_coarse)
    if (is.null(y_dim) || length(y_dim) != 2L) {
        stop("y_coarse must be a matrix.", call. = FALSE)
    }
    for (obj in list(e, x1, x2, lambda_tilde)) {
        if (is.null(dim(obj)) || !identical(as.integer(dim(obj)), as.integer(y_dim))) {
            stop("e, x1, x2, and lambda_tilde must have the same dimensions as y_coarse.",
                 call. = FALSE)
        }
    }
    ...
    prior_mean <- c(priors$beta0_mean, priors$beta_mean)
    prior_sd <- c(priors$beta0_sd, priors$beta_sd)
    if (length(beta_current) != length(prior_mean)) {
        stop("beta_current has incompatible length.", call. = FALSE)
    }

    log_y <- .logpost_beta_s2(beta_current, y_coarse, e, x1, x2,
                              lambda_tilde, phi, r, priors) + log(stats::runif(1))
    ...
    for (step in seq_len(max_steps)) {
        ...
        lp <- .logpost_beta_s2(proposal, y_coarse, e, x1, x2,
                               lambda_tilde, phi, r, priors)
        if (is.finite(lp) && lp >= log_y) {
            return(list(sample = proposal, n_reject = n_reject, accepted = TRUE))
        }
    ...
    list(sample = beta_current, n_reject = n_reject, accepted = FALSE)
}

.logpost_phi_u_s2 <- function(u, B, BHB, tau_phi, y_coarse, e, x1, x2,
...
update_phi_s2 <- function(u_current, B, BHB, tau_phi, y_coarse, e, x1, x2,
                          beta0, beta, lambda_tilde, r, proposal_sd) {
    proposal_sd <- rep(proposal_sd, length.out = length(u_current))
    u_prop <- u_current + stats::rnorm(length(u_current), 0, proposal_sd)
    lp_cur <- .logpost_phi_u_s2(u_current, B, BHB, tau_phi, y_coarse, e, x1, x2,
                                beta0, beta, lambda_tilde, r)
    lp_prop <- .logpost_phi_u_s2(u_prop, B, BHB, tau_phi, y_coarse, e, x1, x2,
                                 beta0, beta, lambda_tilde, r)
...
.logpost_rj_s2 <- function(rj, y, mu, priors) {
    if (!is.finite(rj) || rj <= 0) {
        return(-Inf)
    }
    .nb_loglik_s2(y, mu, rj) +
        stats::dgamma(rj, shape = priors$r_shape, rate = priors$r_rate, log = TRUE)
}

update_r_s2 <- function(r_current, y_coarse, e, x1, x2, lambda_tilde,
                        beta0, beta, phi, priors, mh_sd,
    ...
    for (j in seq_len(n1)) {
        ...
        mu_j <- e[, j] * exp(beta0 + beta[1] * x1[, j] + beta[2] * x2[, j] + phi[j]) *
            lambda_tilde[, j]
        lp_cur <- .logpost_rj_s2(r_current[j], y_coarse[, j], mu_j, priors) + log_r_cur
        lp_prop <- .logpost_rj_s2(r_prop, y_coarse[, j], mu_j, priors) + log_r_prop
