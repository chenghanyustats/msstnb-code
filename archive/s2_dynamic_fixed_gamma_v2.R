...
source_s2_dynamic_fixed_gamma <- function(root = ".", verbose = TRUE) {
    ...
    ## Important: Scenario 2 deliberately does NOT source or call
    ## update_regression_revised.R or update_dispersion_revised.R.  Those files
    ## define private helper functions with project-level name collisions.
    ## Scenario 2 uses local update_beta_s2(), update_phi_s2(), and update_r_s2()
    ## below, so no update_beta()/update_phi()/update_r() helper can be called.
    if (isTRUE(verbose)) {
        message("Scenario 2 local-updater version loaded: no update_beta/update_phi/update_r calls.")
    }
