## ======================================================================
## sampler.R
## Revised sampler: beta/phi updated with kappa collapsed out.
##
## Main design changes relative to the original sampler:
## 1. beta and phi target the marginal NB likelihood, not the complete-data
##    Poisson likelihood conditional on sampled kappa.
## 2. kappa is updated AFTER baseline mean structure (beta, phi, tau_phi)
##    so its draw is conditional on the refreshed xi.
## 3. r is still updated from kappa as before.
##
## This redesign is motivated by the diagnostics showing that free kappa was
## absorbing x1 / mean-level structure and distorting beta, phi, and lambda.
## ======================================================================
...
initialise_state <- function(dat, settings, priors, spatial, constants) {
    ...
    list(beta0 = beta0, beta = beta, phi = phi, u = u,
         ...
         lambda_tilde = lambda_tilde, omega = omega, xi = xi,
         ess_base = ess_base,
...
run_one_iteration <- function(state, dat, settings, priors, spatial, constants) {
    ...
    ## Step 2 (moved here): kappa exact conditional given refreshed baseline
    if (settings$include_nb) {
...
run_mcmc <- function(dat, settings, priors, spatial, constants,
    ...
    list(samples = samples, diagnostics = diag_all,
         ...
         method = method_label(settings), n_stored = n_stored)
}
