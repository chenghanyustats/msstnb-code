## ======================================================================
## sampler_common_gamma.R
## Revised sampler with a single common gamma shared across coarsest regions.
##
## Relative to sampler.R, the only substantive changes are:
##   1. gamma is scalar in the state, samples, and diagnostics
##   2. update_gamma_common() is used instead of update_gamma()
##   3. ffbs_lambda_all() receives rep(gamma, n1)
## ======================================================================
...
initialise_state_common_gamma <- function(dat, settings, priors, spatial, constants) {
    TT <- dat$TT
    n1 <- dat$n1
    K  <- dat$n_children
...
run_one_iteration_common_gamma <- function(state, dat, settings, priors, spatial, constants) {
    diag <- list()
    ...
    gamma_result <- update_gamma_common(
    ...
    state$lambda_tilde <- ffbs_lambda_all(
        rep(state$gamma, dat$n1), dat$y_coarse, state$xi, state$kappa,
        a0 = constants$A0, b0 = constants$B0
    )
...
run_mcmc_common_gamma <- function(dat, settings, priors, spatial, constants,
                                  verbose = 1000L) {
    ...
    diag_all <- list(
        ...
        gamma_accept_trace = logical(n_iter),
        delta_accept_trace = logical(n_iter),
        r_accept_trace     = matrix(FALSE, n_iter, n1)
    )

    state <- initialise_state_common_gamma(dat, settings, priors, spatial, constants)
    ...
    for (iter in seq_len(n_iter)) {
        result <- run_one_iteration_common_gamma(state, dat, settings, priors, spatial, constants)
        state <- result$state
        diag <- result$diag
        ...
        if (!is.null(diag$gamma_accept)) {
            ...
            if (diag$gamma_accept) gamma_accept_window <- gamma_accept_window + 1L
        }
        ...
        if (verbose > 0 && iter %% verbose == 0) {
            ...
            gamma_rate <- mean(diag_all$gamma_accept_trace[i0:iter])
            beta_rej <- mean(diag_all$beta_n_reject[i0:iter])
            cat(sprintf(
                "  iter %5d/%d [%.0fs] loglik=%.1f beta0=%.3f gamma=%.3f | phi_acc=%.2f gamma_acc=%.2f beta_rej=%.1f\n",
                iter, n_iter, elapsed, diag$loglik,
                state$beta0, state$gamma,
    ...
    diag_all$gamma_accept_rate      <- mean(diag_all$gamma_accept_trace)
    ...
    result <- list(
        ...
        settings = settings,
        priors = priors,
        method = paste0(method_label(settings), "_common_gamma"),
        n_stored = n_stored,
        gamma_variant = "common"
    )
