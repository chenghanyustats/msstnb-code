...
MCMC_PRIORS <- list(
    ...
    gamma_a = 5,              # Beta(a_γ, b_γ) — mildly favoring near 1
    gamma_b = 1,
...
MCMC_SETTINGS <- list(
    ...
    ## ---- MH tuning (proposal standard deviations) ----
    ## These are on the transformed scales (logit for γ/δ, log for r)
    mh_sd_gamma = 0.3,       # sd for logit(γ) proposal
    mh_sd_delta = 0.3,       # sd for logit(δ) proposal
    mh_sd_log_r = 0.5,       # sd for log(r) proposal
