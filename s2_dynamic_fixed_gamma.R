...
## Version: cleaned Scenario 2 core script. Uses package-level update_beta() with optional lambda_tilde.
##
...
source_s2_dynamic_fixed_gamma <- function(root = ".", verbose = TRUE) {
    ...
    ## Scenario 2 now uses the package-level update_beta(), which supports
    ## lambda_tilde = NULL for Scenario 1 and sampled lambda_tilde for Scenario 2.
    ## We keep local update_phi_s2() and update_r_s2() because they are small
    ## Scenario-2-specific wrappers around the same marginal NB likelihood.
    if (isTRUE(verbose)) {
        message("Scenario 2 cleaned version loaded: using package-level update_beta(lambda_tilde = ...).")
    }
...
## optional lambda_tilde.  The local update_phi_s2() and update_r_s2() below
## are retained as compact Scenario-2-specific wrappers around the same marginal
