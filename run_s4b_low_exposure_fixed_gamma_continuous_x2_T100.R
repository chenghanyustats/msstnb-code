## ============================================================================
## run_s4b_low_exposure_fixed_gamma_continuous_x2_T100.R
##
## Scenario 4B low/heterogeneous-exposure stress test with continuous-time x2 and gamma fixed.
##
## Purpose
...
##   data_s4b_low_exposure_continuous_x2/S4B_LOW_HETEROGENEOUS_EXPOSURE_CONTINUOUS_X2_T100/data_repXX.rds
##
## Main outputs
##   output_s4b_low_exposure_continuous_x2/
##     S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_CONTINUOUS_X2_T100/
##
## Notes
...
s4b_data_file <- "s4b_low_exposure_continuous_x2.R"
...
check_s4b_source_dataset <- function(data_file,
    ...
    assert_true_s4b_fit(
        isTRUE(all.equal(dat$x2_ar, expected_x2_ar)),
        paste0("x2_ar is not ", expected_x2_ar, ". Got ", dat$x2_ar, ".")
    )
    assert_true_s4b_fit(
        isTRUE(all.equal(dat$x2_innov_sd, expected_x2_innov_sd)),
        paste0("x2_innov_sd is not ", expected_x2_innov_sd, ". Got ", dat$x2_innov_sd, ".")
    )
    if (exists("validate_s4b_low_exposure_continuous_x2_data", mode = "function")) {
    ...
    if (exists("validate_s4b_low_exposure_continuous_x2_data", mode = "function")) {
        validate_s4b_low_exposure_continuous_x2_data(dat)
    } else if (exists("validate_s4b_low_exposure_data", mode = "function")) {
    ...
    list(
        ...
        x2_ar = dat$x2_ar %||% NA_real_,
        x2_innov_sd = dat$x2_innov_sd %||% NA_real_,
...
fit_s4b_low_exposure_fixed_gamma_continuous_x2_one_rep <- function(rep_id,
                                                     scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_CONTINUOUS_X2_T100",
                                                     data_scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_CONTINUOUS_X2_T100",
                                                     data_dir = "data_s4b_low_exposure_continuous_x2",
                                                     output_dir = "output_s4b_low_exposure_continuous_x2",
    ...
    fit$metadata$model <- "S4B low-heterogeneous-exposure continuous-time-x2 dynamic NB-ICAR with fixed gamma"
    ...
    fit$metadata <- c(
        ...
        list(
            ...
            fit_file_prefix = "fit_S4B_low_exposure_fixed_gamma_continuous_x2_rep",
    ...
    if (isTRUE(save_result)) {
        ...
        fit_file <- file.path(out_dir, paste0("fit_S4B_low_exposure_fixed_gamma_continuous_x2_rep", rr, ".rds"))
        csv_file <- file.path(out_dir, paste0("summary_S4B_low_exposure_fixed_gamma_continuous_x2_rep", rr, ".csv"))
...
fit_s4b_low_exposure_fixed_gamma_continuous_x2_batch <- function(reps = 1:10,
                                                   scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_CONTINUOUS_X2_T100",
                                                   data_scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_CONTINUOUS_X2_T100",
                                                   data_dir = "data_s4b_low_exposure_continuous_x2",
                                                   output_dir = "output_s4b_low_exposure_continuous_x2",
    ...
    for (rep_id in reps) {
        ...
        fit <- fit_s4b_low_exposure_fixed_gamma_continuous_x2_one_rep(
            rep_id = rep_id,
            scenario_id = scenario_id,
            data_scenario_id = data_scenario_id,
            data_dir = data_dir,
    ...
    summary_file <- file.path(out_dir, "summary_S4B_low_exposure_fixed_gamma_continuous_x2_all_reps.csv")
...
print_s4b_fit_driver_summary <- function(source_manifest, fit_summary, fit_manifest) {
    cat("\n=== Scenario 4B low-heterogeneous-exposure continuous-time-x2 fixed-gamma fitting summary ===\n")
...
fit_summary <- fit_s4b_low_exposure_fixed_gamma_continuous_x2_batch(
    reps = reps_formal,
    scenario_id = scenario_id,
    data_scenario_id = data_scenario_id,
    data_dir = data_dir,
...
fit_files <- file.path(
    fit_dir_full,
    sprintf("fit_S4B_low_exposure_fixed_gamma_continuous_x2_rep%02d.rds", as.integer(reps_formal))
)
...
saveRDS(
    ...
    file.path(fit_dir_full, "run_info_S4B_low_exposure_fixed_gamma_continuous_x2_T100.rds")
)
...
cat("\nScenario 4B low-heterogeneous-exposure continuous-time-x2 fixed-gamma T100 fitting finished successfully.\n")
