## ============================================================================
## run_s4a_oracle_lambda_fixed_gamma_diagnostic_continuous_x2_T100.R
##
## Scenario 4A diagnostic: Oracle lambda + fixed gamma.
##
## Purpose
##   Diagnose whether the S4A sparse-count instability in reps 01, 04, 08, and 10 is
...
##
## Interpretation
##   - If reps 01, 04, 08, and 10 become stable with oracle lambda, the original failures
...
scenario_id <- "S4A_ORACLE_LAMBDA_FIXED_GAMMA_CONTINUOUS_X2_DIAGNOSTIC_T100"
...
output_dir <- "output_s4a_oracle_lambda_fixed_gamma_continuous_x2"
verbose <- 1000L
overwrite_fit <- TRUE
...
run_s4a_oracle_lambda_fixed_gamma_mcmc <- function(dat,
    ...
    list(
        ...
        metadata = list(
            model = "S4A oracle-lambda fixed-gamma continuous-time x2 diagnostic",
            fixed_lambda = TRUE,
            oracle_lambda = TRUE,
            oracle_lambda_source = oracle_lambda_field,
            dynamic_lambda = TRUE,
...
fit_s4a_oracle_lambda_one_rep <- function(rep_id,
    ...
    cat(sprintf("=== S4A oracle-lambda fixed-gamma continuous-time x2 diagnostic: rep %s ===\n", rr))
...
fit_s4a_oracle_lambda_batch <- function(reps = reps_formal,
    ...
    run_info_file <- file.path(out_dir, "run_info_S4A_oracle_lambda_fixed_gamma_continuous_x2_diagnostic_T100.rds")
...
cat("\nS4A oracle-lambda fixed-gamma continuous-time x2 diagnostic finished successfully.\n")
