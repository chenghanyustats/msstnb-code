## ============================================================================
## run_s4b_oracle_lambda_fixed_gamma_diagnostic_T100.R
##
## Scenario 4B diagnostic: Oracle lambda + fixed gamma.
##
## Purpose
##   Diagnose whether the S4B low/heterogeneous-exposure instability in reps 04 and 07 is
##   mainly caused by a beta--lambda ridge.  This script fixes lambda_tilde at
##   the true identified lambda path from the S4B data and fixes gamma at truth
...
##
## Main outputs
##   output_s4b_oracle_lambda_fixed_gamma/
##     S4B_ORACLE_LAMBDA_FIXED_GAMMA_DIAGNOSTIC_T100/
## ============================================================================

## ---- user settings ----------------------------------------------------------
root_dir <- "."

scenario_id <- "S4B_ORACLE_LAMBDA_FIXED_GAMMA_DIAGNOSTIC_T100"
...
output_dir <- "output_s4b_oracle_lambda_fixed_gamma"
verbose <- 1000L
overwrite_fit <- TRUE
...
## ---- oracle-lambda MCMC -----------------------------------------------------
run_s4b_oracle_lambda_fixed_gamma_mcmc <- function(dat,
    ...
    list(
        ...
        metadata = list(
            model = "S4B oracle-lambda fixed-gamma diagnostic",
            fixed_lambda = TRUE,
            oracle_lambda = TRUE,
            oracle_lambda_source = oracle_lambda_field,
            dynamic_lambda = TRUE,
...
fit_s4b_oracle_lambda_one_rep <- function(rep_id,
    ...
    cat(sprintf("=== S4B oracle-lambda fixed-gamma diagnostic: rep %s ===\n", rr))
    ...
    reset_oracle_guards()
    fit <- run_s4b_oracle_lambda_fixed_gamma_mcmc(
        dat = dat,
        settings = settings,
        priors = priors,
        spatial = spatial,
    ...
    fit$diagnostics$s4b_oracle_numeric_guards <- guard_counts
    fit$diagnostics$s4b_oracle_beta_guard_count <- guard_counts$n_beta_guard
    fit$diagnostics$s4b_oracle_kappa_guard_count <- guard_counts$n_kappa_guard
    ...
    summary$s4b_oracle_beta_guard_count <- guard_counts$n_beta_guard
    summary$s4b_oracle_kappa_guard_count <- guard_counts$n_kappa_guard
    ...
    fit$metadata <- c(
        ...
        list(
            ...
            fit_file_prefix = "fit_S4B_oracle_lambda_fixed_gamma_rep",
            run_time = Sys.time()
        )
    )
    ...
    if (isTRUE(save_result)) {
        ...
        ensure_dir(out_dir)
        fit_file <- file.path(out_dir, paste0("fit_S4B_oracle_lambda_fixed_gamma_rep", rr, ".rds"))
        csv_file <- file.path(out_dir, paste0("summary_S4B_oracle_lambda_fixed_gamma_rep", rr, ".csv"))
...
fit_s4b_oracle_lambda_batch <- function(reps = reps_formal,
                                        scenario_id_arg = scenario_id,
                                        data_scenario_id_arg = data_scenario_id,
                                        data_dir_arg = data_dir,
                                        output_dir_arg = output_dir,
    ...
    for (rep_id in reps) {
        ...
        if (file.exists(fit_file) && !isTRUE(overwrite_existing)) {
            ...
            fit <- fit_s4b_oracle_lambda_one_rep(
                rep_id = rep_id,
    ...
    summary_file <- file.path(out_dir, "summary_S4B_oracle_lambda_fixed_gamma_all_reps.csv")
    manifest_file <- file.path(out_dir, "s4b_oracle_fit_manifest.csv")
    run_info_file <- file.path(out_dir, "run_info_S4B_oracle_lambda_fixed_gamma_diagnostic_T100.rds")
    ...
    cat("\n=== S4B oracle-lambda diagnostic summary ===\n")
    print(summary_all[, intersect(c(
        ...
        "s4b_oracle_beta_guard_count", "s4b_oracle_kappa_guard_count"
...
s4b_oracle_summary <- fit_s4b_oracle_lambda_batch(
    reps = reps_formal,
    scenario_id_arg = scenario_id,
    data_scenario_id_arg = data_scenario_id,
    data_dir_arg = data_dir,
...
cat("\nS4B oracle-lambda fixed-gamma diagnostic finished successfully.\n")
