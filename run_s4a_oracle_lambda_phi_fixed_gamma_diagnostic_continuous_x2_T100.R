## ============================================================================
## run_s4a_oracle_lambda_phi_fixed_gamma_diagnostic_continuous_x2_T100.R
##
## Scenario 4A diagnostic: Oracle lambda + oracle phi + fixed gamma.
##
## Purpose
##   Diagnose whether the remaining S4A instability after oracle-lambda fitting
##   is caused by beta0--phi / spatial latent-effect confounding or by the
##   beta/r likelihood block itself.  This script fixes lambda_tilde and phi at
##   their true identified values and fixes gamma at truth (0.8), while
##   estimating beta0, beta1, beta2, r, and kappa.
##
## Interpretation
...
##   data_s4a_sparse_counts/S4A_SPARSE_COUNTS_CONTINUOUS_X2_T100/data_rep04.rds
##   data_s4a_sparse_counts/S4A_SPARSE_COUNTS_CONTINUOUS_X2_T100/data_rep07.rds
##
## Main outputs
...
scenario_id <- "S4A_ORACLE_LAMBDA_PHI_FIXED_GAMMA_CONTINUOUS_X2_DIAGNOSTIC_T100"
data_scenario_id <- "S4A_SPARSE_COUNTS_CONTINUOUS_X2_T100"

## This diagnostic targets the continuous-time x2 S4A numerical-instability reps.
reps_formal <- c(1L, 4L, 8L, 10L)
...
output_dir <- "output_s4a_oracle_lambda_phi_fixed_gamma_continuous_x2"
verbose <- 1000L
overwrite_fit <- TRUE
...
check_s4a_oracle_source_dataset <- function(data_file,
    ...
    required <- c(
        ...
        "mean_count", "zero_prop", "TT", "n1", "x2_mode", "x2_ar", "x2_innov_sd"
    )
    ...
    assert_true(identical(dat$x2_mode, expected_x2_mode),
                paste0("x2_mode is not ", expected_x2_mode, "."))
    assert_true(abs(dat$x2_ar - expected_x2_ar) < 1e-12,
                paste0("x2_ar is not ", expected_x2_ar, "."))
    assert_true(abs(dat$x2_innov_sd - expected_x2_innov_sd) < 1e-12,
                paste0("x2_innov_sd is not ", expected_x2_innov_sd, "."))
    x2_vals <- as.numeric(dat$x2)
    x2_binary_like_prop <- mean(abs(x2_vals - round(x2_vals)) < 1e-8 & x2_vals %in% c(0, 1))
    assert_true(is.finite(stats::sd(x2_vals)) && stats::sd(x2_vals) > 0.2,
                "x2 has too little variation for continuous-time design.")
    assert_true(x2_binary_like_prop < 0.95,
                "x2 appears binary-like, not continuous-time.")
...
## ---- oracle-lambda MCMC -----------------------------------------------------
run_s4a_oracle_lambda_phi_fixed_gamma_mcmc <- function(dat,
    ...
    list(
        ...
        metadata = list(
            model = "S4A oracle-lambda-phi fixed-gamma diagnostic",
            fixed_lambda = TRUE,
            fixed_phi = TRUE,
            oracle_lambda = TRUE,
            oracle_phi = TRUE,
...
fit_s4a_oracle_lambda_phi_one_rep <- function(rep_id,
    ...
    cat(sprintf("=== S4A oracle-lambda-phi fixed-gamma diagnostic: rep %s ===\n", rr))
    ...
    reset_oracle_guards()
    fit <- run_s4a_oracle_lambda_phi_fixed_gamma_mcmc(
        dat = dat,
        settings = settings,
        priors = priors,
        spatial = spatial,
    ...
    summary$s4a_oracle_lamphi_beta_guard_count <- guard_counts$n_beta_guard
    summary$s4a_oracle_lamphi_kappa_guard_count <- guard_counts$n_kappa_guard
    ...
    fit$metadata <- c(
        ...
        list(
            ...
            fit_file_prefix = "fit_S4A_oracle_lambda_phi_fixed_gamma_rep",
            run_time = Sys.time()
        )
    )
    ...
    if (isTRUE(save_result)) {
        ...
        ensure_dir(out_dir)
        fit_file <- file.path(out_dir, paste0("fit_S4A_oracle_lambda_phi_fixed_gamma_rep", rr, ".rds"))
        csv_file <- file.path(out_dir, paste0("summary_S4A_oracle_lambda_phi_fixed_gamma_rep", rr, ".csv"))
...
fit_s4a_oracle_lambda_phi_batch <- function(reps = reps_formal,
                                        scenario_id_arg = scenario_id,
                                        data_scenario_id_arg = data_scenario_id,
                                        data_dir_arg = data_dir,
                                        output_dir_arg = output_dir,
    ...
    for (rep_id in reps) {
        ...
        if (file.exists(fit_file) && !isTRUE(overwrite_existing)) {
            ...
            fit <- fit_s4a_oracle_lambda_phi_one_rep(
                rep_id = rep_id,
    ...
    summary_file <- file.path(out_dir, "summary_S4A_oracle_lambda_phi_fixed_gamma_all_reps.csv")
    manifest_file <- file.path(out_dir, "s4a_oracle_lamphi_fit_manifest.csv")
    run_info_file <- file.path(out_dir, "run_info_S4A_oracle_lambda_phi_fixed_gamma_continuous_x2_diagnostic_T100.rds")
    ...
    cat("\n=== S4A oracle-lambda-phi diagnostic summary ===\n")
...
s4a_oracle_lambda_phi_summary <- fit_s4a_oracle_lambda_phi_batch(
    reps = reps_formal,
    scenario_id_arg = scenario_id,
    data_scenario_id_arg = data_scenario_id,
    data_dir_arg = data_dir,
...
cat("\nS4A oracle-lambda-phi fixed-gamma diagnostic finished successfully.\n")
