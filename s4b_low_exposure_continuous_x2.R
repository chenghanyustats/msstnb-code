## ============================================================================
## s4b_low_exposure_continuous_x2.R
## Scenario 4B low-exposure data-generation script with continuous-time x2.
##
## Scenario 4B purpose:
...
## ============================================================================
## Continuous-time x2 wrapper for revised Scenario 4B
## ============================================================================
## Revised S4B official covariate design:
##   x2_mode     = "continuous_time"
##   x2_ar       = 0.50
##   x2_innov_sd = 0.80
##
## These functions intentionally keep the original low/heterogeneous exposure
## mechanism unchanged and only force the covariate design to match S1--S3 and
## the revised S4A/S4D workflows.
## ============================================================================

.summarise_s4b_x2_design <- function(dat) {
    x2 <- dat$x2
    if (is.null(x2)) {
        return(list(
            x2_found = FALSE,
...
validate_s4b_low_exposure_continuous_x2_data <- function(dat,
    ...
    if (length(missing) > 0L) {
        stop(
            "S4B continuous-time x2 data is missing fields: ",
    ...
    xs <- .summarise_s4b_x2_design(dat)
    if (isTRUE(strict)) {
...
summarise_s4b_low_exposure_counts_one_continuous_x2 <- function(dat) {
    validate_s4b_low_exposure_continuous_x2_data(dat)
    base <- summarise_s4b_low_exposure_counts_one(dat)
    xs <- .summarise_s4b_x2_design(dat)
    ...
    ## Place x2 fields immediately after the exposure-design fields when possible.
    cbind(base, x2_df)
}

simulate_s4b_low_exposure_continuous_x2_batch <- function(reps = 1:10,
                                                          data_dir = "data_s4b_low_exposure_continuous_x2",
                                                          scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_CONTINUOUS_X2_T100",
                                                          root = ".",
                                                          seed_base = NULL,
                                                          overwrite_existing = TRUE,
                                                          verbose = TRUE,
    ...
    if (!identical(x2_mode, "continuous_time")) {
        stop("For revised S4B, x2_mode must be exactly 'continuous_time'.", call. = FALSE)
    }
    if (!isTRUE(all.equal(x2_ar, 0.50))) {
        stop("For revised S4B, x2_ar must be 0.50.", call. = FALSE)
    }
    if (!isTRUE(all.equal(x2_innov_sd, 0.80))) {
        stop("For revised S4B, x2_innov_sd must be 0.80.", call. = FALSE)
    }
    ...
    for (rep_id in reps) {
        ...
        if (file.exists(out_file) && !isTRUE(overwrite_existing)) {
            ...
            ## only if they are already consistent, and then validate strictly.
            ...
            dat$s4b_continuous_x2_config <- list(
                x2_mode = x2_mode,
                x2_ar = x2_ar,
                x2_innov_sd = x2_innov_sd,
                target_mean_multiplier = dat$target_mean_exposure_multiplier,
                area_log_sd = dat$exposure_area_log_sd,
                time_log_sd = dat$exposure_time_log_sd,
                lower_multiplier = dat$exposure_lower_multiplier,
                upper_multiplier = dat$exposure_upper_multiplier
            )
            dat$scenario_info <- c(dat$scenario_info %||% list(), list(
                scenario_family = "S4B",
                covariate_design = "continuous_time_x2",
                stress_test = "low_heterogeneous_exposure"
            ))
            ...
            if (isTRUE(verbose)) {
                ...
                message(sprintf(
                    paste0(
                        "Saved %s | mean_count=%.2f zero_prop=%.3f ",
                        "mean_mult=%.4f exposure_cv=%.3f x2_ar_emp=%.3f"
                    ),
                    out_file, dat$mean_count, dat$zero_prop,
                    dat$realized_mean_exposure_multiplier,
                    dat$exposure_cv,
                    xs$x2_empirical_ar1_mean
                ))
            }
        }

        row <- summarise_s4b_low_exposure_counts_one_continuous_x2(dat)
        row$file <- out_file
        manifest[[rr]] <- row

        group_row <- exposure_group_summary_s4b(dat, n_groups = 3L)
        group_row$x2_mode <- dat$x2_mode
        group_row$x2_ar <- dat$x2_ar
        group_row$x2_innov_sd <- dat$x2_innov_sd
        group_row$file <- out_file
        group_manifest[[rr]] <- group_row
    }
    ...
    if (isTRUE(verbose)) {
        ...
        message(sprintf(
            paste0(
                "S4B continuous-time x2 summary | reps=%d mean_count=%.2f zero_prop=%.3f ",
                "mean_multiplier=%.4f exposure_cv=%.3f x2_emp_ar=%.3f"
            ),
            nrow(manifest_df),
            mean(manifest_df$mean_count),
            mean(manifest_df$zero_prop),
...
run_s4b_low_exposure_continuous_x2_data_generation <- function(root = ".",
                                                               s3_script = NULL,
                                                               reps = 1:10,
                                                               TT_use = 100,
                                                               target_mean_multiplier = 0.05,
    ...
    simulate_s4b_low_exposure_continuous_x2_batch(
        reps = reps,
        data_dir = data_dir,
        scenario_id = scenario_id,
        root = root,
...
check_s4b_low_exposure_continuous_x2_data_summary <- function(manifest,
                                                              target_mean_count_range = c(5, 20),
                                                              target_zero_prop_range = c(0.05, 0.35),
                                                              target_mean_multiplier_range = c(0.03, 0.08),
                                                              minimum_exposure_cv = 0.40,
    ...
    out <- check_s4b_low_exposure_data_summary(
        manifest,
        target_mean_count_range = target_mean_count_range,
        target_zero_prop_range = target_zero_prop_range,
        target_mean_multiplier_range = target_mean_multiplier_range,
        minimum_exposure_cv = minimum_exposure_cv,
...
## Short alias matching the revised workflow name.
run_s4b_continuous_x2_data_generation <- run_s4b_low_exposure_continuous_x2_data_generation
check_s4b_continuous_x2_data_summary <- check_s4b_low_exposure_continuous_x2_data_summary

## Example revised usage:
##
## source("s4b_low_exposure_continuous_x2.R")
## manifest <- run_s4b_continuous_x2_data_generation(
##     root = ".",
##     reps = 1:10,
##     TT_use = 100,
##     target_mean_multiplier = 0.05,
...
##     overwrite_existing = TRUE
## )
## check_s4b_continuous_x2_data_summary(manifest)
