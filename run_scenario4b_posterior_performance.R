## ============================================================================
## VERSION: S4B_POSTERIOR_PERFORMANCE_V2_WITH_ORACLE_DIAGNOSTICS_2026_06_18
## run_scenario4b_posterior_performance.R
##
## Posterior-performance and failure-mode summaries for Scenario 4B:
## low and heterogeneous exposure stress test with fixed gamma.
##
## Design goal
...
##   source("run_scenario4b_posterior_performance.R")
##
## Or programmatically:
...
.s4b_make_beta_long <- function(d, scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_T100") {
...
.s4b_make_lambda_summary <- function(d, scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_T100") {
    cols <- c(
...
.s4b_make_exposure_group_long <- function(d, scenario_id = "S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_T100") {
...
.s4b_build_comparison_table <- function(s4b_diag_agg,
    ...
    if (!is.null(s4a_performance_subset) && nrow(s4a_performance_subset) > 0L) {
        s4a_row <- s4a_performance_subset[s4a_performance_subset$subset == "stable_plus_soft_warning", , drop = FALSE]
        if (nrow(s4a_row) == 0L) s4a_row <- s4a_performance_subset[1L, , drop = FALSE]
        inst_rate <- NA_real_
        if (!is.null(s4a_status_counts) && nrow(s4a_status_counts) > 0L) {
        ...
        rows[["s4a"]] <- data.frame(
            scenario = "S4A sparse counts fixed gamma",
            ...
            beta2_mean_sd = .s4b_num(s4a_row$beta2_mean_sd),
            lambda_rmse_avg = .s4b_num(s4a_row$lambda_rmse_avg),
            log_lambda_rmse_avg = .s4b_num(s4a_row$log_lambda_rmse_avg),
            cor_log_lambda_avg = .s4b_num(s4a_row$cor_log_lambda_avg),
            numerical_instability_rate = inst_rate,
            conclusion = "Global sparse-count stress; beta2 and latent recovery degrade and some reps fail.",
            stringsAsFactors = FALSE
        )
    }
    ...
    rows[["s4b"]] <- data.frame(
        scenario = "S4B low heterogeneous exposure fixed gamma",
...
summarize_scenario4b_posterior_performance <- function(root = ".",
                                                       summary_file = "output_s4b_low_exposure_fixed_gamma/S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_T100/summary_S4B_low_exposure_fixed_gamma_all_reps.csv",
                                                       analysis_dir = "analysis_s4b_low_exposure/S4B_LOW_HETEROGENEOUS_EXPOSURE_FIXED_GAMMA_T100",
                                                       ...
                                                       s4a_performance_subset_file = "analysis_s4a_sparse_counts/S4A_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100/tables/scenario4a_performance_by_subset.csv",
                                                       s4a_status_counts_file = "analysis_s4a_sparse_counts/S4A_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100/tables/scenario4a_fit_status_counts.csv",
                                                       oracle_lambda_summary_file = "output_s4b_oracle_lambda_fixed_gamma/S4B_ORACLE_LAMBDA_FIXED_GAMMA_DIAGNOSTIC_T100/summary_S4B_oracle_lambda_fixed_gamma_all_reps.csv",
                                                       oracle_lamphi_summary_file = "output_s4b_oracle_lambda_phi_fixed_gamma/S4B_ORACLE_LAMBDA_PHI_FIXED_GAMMA_DIAGNOSTIC_T100/summary_S4B_oracle_lambda_phi_fixed_gamma_all_reps.csv",
                                                       make_plots = TRUE,
...
run_scenario4b_posterior_performance <- function(root = ".",
                                                 ...
                                                 analysis_dir = "analysis_s4b_low_exposure",
                                                 make_plots = TRUE,
                                                 plot_format = c("pdf", "png"),
                                                 verbose = TRUE,
                                                 ...) {
    plot_format <- match.arg(plot_format)
    summary_file <- file.path(output_dir, scenario_id, "summary_S4B_low_exposure_fixed_gamma_all_reps.csv")
