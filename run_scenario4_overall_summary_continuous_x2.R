# OFFICIAL FINAL 2026-06-23: Scenario 4 overall summary, continuous-time x2.
# Includes robust S4D continuous-time path detection and corrected figures.
# ================================================================
# Scenario 4 overall stress-test summary, continuous-time x2 version (final)
# ================================================================
#
# Purpose:
...
.s4overall_plot_bar <- function(path, names, values, main, ylab, width = 8, height = 5,
                                las = 2, omit_missing = FALSE, missing_note = NULL,
                                hline = NULL, hline_label = NULL) {
    # Robust plotting helper.  It handles negative-only metrics, all-zero metrics,
    # and optionally omits missing values instead of plotting them as zero.
    vals <- .s4overall_num(values)
    labs <- as.character(names)
    if (length(labs) != length(vals)) labs <- seq_along(vals)

    finite <- is.finite(vals)
    if (isTRUE(omit_missing)) {
        labs <- labs[finite]
        vals <- vals[finite]
        finite <- is.finite(vals)
    }
    ...
    y_min <- min(0, plot_vals, hline, na.rm = TRUE)
    y_max <- max(0, plot_vals, hline, na.rm = TRUE)
    ...
    if (!is.null(hline) && is.finite(hline)) {
        graphics::abline(h = hline, lty = 2)
        if (!is.null(hline_label) && length(bp) > 0L) {
            graphics::text(x = max(bp), y = hline, labels = hline_label, pos = 3, cex = 0.75)
        }
    }
    if (!isTRUE(omit_missing) && any(!finite)) {
        graphics::mtext("Missing values plotted as 0", side = 1, line = 6.8, cex = 0.7)
    } else if (!is.null(missing_note)) {
        graphics::mtext(missing_note, side = 1, line = 6.8, cex = 0.7)
    }
}
...
.s4overall_plot_status <- function(path, stability_summary, width = 8, height = 5) {
    ...
    m <- rbind(
        stable = .s4overall_num(stability_summary$n_stable),
        soft_warning = .s4overall_num(stability_summary$n_soft_warning),
        numerical_instability = .s4overall_num(stability_summary$n_numerical_instability)
    )
    m[!is.finite(m)] <- 0
...
.s4overall_default_config <- function(root) {
    ...
    list(
        ...
        S4D = list(
            ...
            table_dirs = c(
                p("analysis_s4d_short_time_continuous_x2", "S4D_SHORT_TIME_FIXED_GAMMA_T30_CONTINUOUS_X2", "tables"),
                p("analysis_s4d_short_time_continuous_x2", "S4D_SHORT_TIME_FIXED_GAMMA_T30_CONTINUOUS_X2", "fits"),
...
.s4overall_make_figures <- function(summary_table, fig_dir, formats = c("png", "pdf"), verbose = TRUE) {
    ...
    for (ext in formats) {
        .s4overall_plot_bar(
            ...
            omit_missing = TRUE,
            missing_note = "S4A omitted: unstable sparse-count scenario has no primary stable log-lambda summary"
        )
    }
