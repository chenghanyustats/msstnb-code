## ============================================================================
## run_s4c_oracle_lambda_fixed_gamma_diagnostic_T100.R
##
## Scenario 4C diagnostic: Oracle lambda + fixed gamma.
##
## Purpose
##   Diagnose whether the S4C strong-overdispersion instability in rep 07 is
##   mainly caused by a beta--lambda / latent-temporal-risk ridge.  This script
##   fixes lambda_tilde at the true identified lambda path from the S4C data and
##   fixes gamma at truth (0.8), while estimating beta0, beta1, beta2, phi,
##   tau_phi, r, and kappa.
##
## Interpretation
##   - If rep 07 becomes stable with oracle lambda, the sampled-lambda failure
##     is strong evidence of beta--lambda confounding / weak temporal-risk
##     separation under strong overdispersion.
##   - If beta2 is restored but beta0/phi/kappa remain unstable, the next
##     diagnostic should fix both lambda and phi and estimate beta/r/kappa.
##   - If the beta/r/kappa block remains poor even with oracle lambda and oracle
##     phi, then the failure is no longer only a latent-effect ridge.
##
...
scenario_id <- "S4C_ORACLE_LAMBDA_FIXED_GAMMA_DIAGNOSTIC_T100"
data_scenario_id <- "S4C_STRONG_OVERDISPERSION_T100"

## This diagnostic targets the one numerical-instability S4C replicate.
reps_formal <- c(7L)

## Use the same formal MCMC profile as S4C fixed-gamma runs.
n_iter <- 40000L
n_burnin <- 20000L
n_thin <- 5L
...
output_dir <- "output_s4c_oracle_lambda_fixed_gamma"
verbose <- 1000L
overwrite_fit <- TRUE
...
cat(sprintf(
    "Using S4C oracle-lambda diagnostic guards: beta0 in [%.1f, %.1f], beta in [%.1f, %.1f], log(xi) in [%.1f, %.1f].\n",
...
## ---- oracle-lambda MCMC -----------------------------------------------------
run_s4c_oracle_lambda_fixed_gamma_mcmc <- function(dat,
    ...
    list(
        ...
        metadata = list(
            model = "S4C oracle-lambda fixed-gamma diagnostic",
            fixed_lambda = TRUE,
            oracle_lambda = TRUE,
            oracle_lambda_source = oracle_lambda_field,
            dynamic_lambda = TRUE,
...
## ---- fit wrapper ------------------------------------------------------------
fit_s4c_oracle_lambda_one_rep <- function(rep_id,
    ...
    cat(sprintf("=== S4C oracle-lambda fixed-gamma diagnostic: rep %s ===\n", rr))
    ...
    cat(sprintf("Oracle    : lambda = dat$%s in [%.3f, %.3f]\n",
    ...
    reset_s4c_oracle_guards()
    fit <- run_s4c_oracle_lambda_fixed_gamma_mcmc(
        dat = dat,
        settings = settings,
        priors = priors,
        spatial = spatial,
    ...
    fit$metadata <- c(
        ...
        list(
            ...
            fit_file_prefix = "fit_S4C_oracle_lambda_fixed_gamma_rep",
            run_time = Sys.time()
        )
    )
    ...
    if (isTRUE(save_result)) {
        ...
        fit_file <- file.path(out_dir, paste0("fit_S4C_oracle_lambda_fixed_gamma_rep", rr, ".rds"))
        csv_file <- file.path(out_dir, paste0("summary_S4C_oracle_lambda_fixed_gamma_rep", rr, ".csv"))
...
fit_s4c_oracle_lambda_batch <- function(reps = reps_formal,
                                        scenario_id_arg = scenario_id,
                                        data_scenario_id_arg = data_scenario_id,
                                        data_dir_arg = data_dir,
                                        output_dir_arg = output_dir,
    ...
    for (rep_id in reps) {
        ...
        if (file.exists(fit_file) && !isTRUE(overwrite_existing)) {
            ...
            fit <- fit_s4c_oracle_lambda_one_rep(
                rep_id = rep_id,
    ...
    summary_file <- file.path(out_dir, "summary_S4C_oracle_lambda_fixed_gamma_all_reps.csv")
    manifest_file <- file.path(out_dir, "s4c_oracle_lambda_fit_manifest.csv")
    run_info_file <- file.path(out_dir, "run_info_S4C_oracle_lambda_fixed_gamma_diagnostic_T100.rds")
    ...
    cat("\n=== S4C oracle-lambda diagnostic summary ===\n")
...
s4c_oracle_lambda_summary <- fit_s4c_oracle_lambda_batch(
    reps = reps_formal,
    scenario_id_arg = scenario_id,
    data_scenario_id_arg = data_scenario_id,
    data_dir_arg = data_dir,
...
cat("\nS4C oracle-lambda fixed-gamma diagnostic finished successfully.\n")
