## ============================================================================
## s4a_sparse_counts_continuous_x2.R
## Scenario 4A sparse-count data-generation script for the MSSTNB project.
##
## This is the direct continuous-time-x2 update of
...
##   2. Use the same continuous-time x2 covariate design as S1--S3:
##
##        x2_mode     = "continuous_time"
##        x2_ar       = 0.50
##        x2_innov_sd = 0.80
##
##   3. Keep the same exposure, covariates, phi, lambda_tilde, omega, kappa,
##      gamma, delta, and r.
##   4. Re-generate only the observation counts under a lower baseline intensity:
##
...
source_s4a_sparse_counts_continuous_x2 <- function(root = ".",
                                                   s3_script = NULL,
                                                   verbose = TRUE) {
    source_s4a_sparse_counts_observation(
        root = root,
        s3_script = s3_script,
        verbose = verbose
    )
    if (isTRUE(verbose)) {
        message("Scenario 4A continuous-time x2 sparse-count generator loaded.")
    }
    invisible(TRUE)
}

## -----------------------------------------------------------------------------
## Continuous-time x2 validation helpers
## -----------------------------------------------------------------------------
.call_s3_generator_s4a_obs <- function(...) {
    ...
    ## Some project versions expose only a subset of these controls.  We pass all
    ## controls when possible and otherwise keep only formal arguments.  The
    ## post-generation x2 validation below is the safeguard against silently
    ## using the wrong covariate design.
    args_to_pass <- args[names(args) %in% fml]
    do.call(fun, args_to_pass)
}

.summarise_s4a_x2_design <- function(dat) {
    if (is.null(dat$x2)) {
        return(list(
            x2_found = FALSE,
            x2_mean = NA_real_,
    ...
    x2 <- as.matrix(dat$x2)
    ar_vals <- rep(NA_real_, ncol(x2))
    if (nrow(x2) >= 3L) {
        for (jj in seq_len(ncol(x2))) {
            xx <- x2[, jj]
            if (stats::sd(xx[-length(xx)]) > 0 && stats::sd(xx[-1L]) > 0) {
                ar_vals[jj] <- stats::cor(xx[-length(xx)], xx[-1L])
            }
        }
    }

    near_zero_one <- abs(as.numeric(x2)) < 1e-10 | abs(as.numeric(x2) - 1) < 1e-10

    list(
        x2_found = TRUE,
        x2_mean = mean(x2),
        x2_sd = stats::sd(as.numeric(x2)),
        x2_min = min(x2),
        x2_max = max(x2),
...
validate_s4a_continuous_x2_design <- function(dat,
    ...
    if (length(missing) > 0L) {
        stop("S4A continuous-time x2 data is missing fields: ",
             paste(missing, collapse = ", "), call. = FALSE)
    }
    ...
    xs <- .summarise_s4a_x2_design(dat)
    if (isTRUE(strict)) {
...
simulate_s4a_observation_sparse_one <- function(seed = 1L,
    ...
    if (!identical(x2_mode, "continuous_time")) {
        stop("For revised S4A, x2_mode must be exactly 'continuous_time'.", call. = FALSE)
    }
    if (!isTRUE(all.equal(x2_ar, 0.50))) {
        stop("For revised S4A, x2_ar must be 0.50.", call. = FALSE)
    }
    if (!isTRUE(all.equal(x2_innov_sd, 0.80))) {
        stop("For revised S4A, x2_innov_sd must be 0.80.", call. = FALSE)
    }

    beta0_sparse_truth <- beta0_reference_truth + sparse_beta0_shift

    ## Generate the full Scenario 3 latent structure under the reference intensity.
    ref_dat <- .call_s3_generator_s4a_obs(
        seed = seed,
        TT_use = TT_use,
    ...
    dat$covariate_design <- list(
        ...
        x2_ar = x2_ar,
        x2_innov_sd = x2_innov_sd,
        description = "continuous-time AR(1) x2 covariate used for S1--S3-aligned stress tests"
    )
    dat$s4a_continuous_x2_config <- list(
        ...
        beta0_sparse_truth = beta0_sparse_truth
    )
...
run_s4a_observation_sparse_data_generation <- function(root = ".",
                                                       ...
                                                       x2_innov_sd = 0.80,
                                                       ...) {
    source_s4a_sparse_counts_continuous_x2(root = root, s3_script = s3_script, verbose = verbose)
...
run_s4a_continuous_x2_data_generation <- function(root = ".",
                                                  s3_script = NULL,
                                                  reps = 1:10,
                                                  TT_use = 100,
                                                  sparse_beta0_shift = -4.25,
                                                  ...
                                                  verbose = TRUE,
                                                  ...) {
    run_s4a_observation_sparse_data_generation(
        root = root,
        s3_script = s3_script,
        reps = reps,
        TT_use = TT_use,
