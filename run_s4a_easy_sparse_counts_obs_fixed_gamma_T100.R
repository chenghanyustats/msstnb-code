## ============================================================================
## run_s4a_easy_sparse_counts_obs_fixed_gamma_T100.R
##
## Scenario 4A easier sparse-count observation-level stress test with gamma fixed.
##
## Purpose
##   Fit the same dynamic MSSTNB model used in Scenario 3 to S4A sparse-count
##   observation-level data, but fix the common temporal discount parameter
##   gamma at its truth value (0.8).
##
## Why fixed gamma here?
##   The easier S4A data keep the Scenario 3 latent spatiotemporal structure
##   fixed and only lower the observation-level intensity to create sparse
##   counts.  This intentionally isolates sparse-observation information loss.
##   Direct learned-gamma fitting is a useful diagnostic, but its complete-data
##   gamma update is sensitive to the artificial decoupling between sparse
##   observed counts and the retained reference lambda path.  Therefore the main
##   clean S4A analysis fixes gamma at truth and evaluates recovery of beta,
##   phi, r, and lambda under sparse observations.
##
## Required files in project root
##   s3_dynamic_learned_gamma.R
##   data_s4a_sparse_counts/S4A_EASY_SPARSE_COUNTS_OBS_T100/data_repXX.rds
##
## Main outputs
##   output_s4a_easy_sparse_counts_fixed_gamma/
##     S4A_EASY_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100/
##
## Notes
...
scenario_id <- "S4A_EASY_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100"
data_scenario_id <- "S4A_EASY_SPARSE_COUNTS_OBS_T100"
...
## Core Scenario 3 model code.  S4A intentionally reuses the same dynamic
## sampler, but locally overrides the gamma update to keep gamma fixed.
s3_core_file <- "s3_dynamic_learned_gamma.R"

## Data-generation constants expected in the easier S4A data.
TT_use <- 100L
n1_use <- 9L
expected_stress_type <- "observation_sparse_counts"
expected_sparse_beta0_shift <- -4.0
expected_beta0_reference_truth <- -1.5
expected_beta0_sparse_truth <- -5.5
expected_count_multiplier <- exp(expected_sparse_beta0_shift)

## MCMC settings for fixed-gamma fitting.  These mirror Scenario 3 except gamma is fixed.
if (identical(run_profile, "short_test")) {
    n_iter <- 6000L
    n_burnin <- 1000L
    n_thin <- 5L
...
output_dir <- "output_s4a_easy_sparse_counts_fixed_gamma"
verbose <- 1000L

## Easier S4A data-generation settings.  This script will generate the
## observation-level sparse data with shift = -4.0 if the target data folder is
## missing.  It will not overwrite existing data unless explicitly requested.
s4a_data_generator_file <- "s4a_sparse_counts_observation.R"
generate_easy_data_if_missing <- TRUE
overwrite_easy_data_if_existing <- FALSE
...
## Because v2 adds a numerical xi guard, rerun all existing S4A fixed-gamma
## fits once for consistency.  After a complete successful formal run, you may
## set rerun_all_after_safe_xi_fix <- FALSE to skip existing fits in later runs.
rerun_all_after_safe_xi_fix <- TRUE
overwrite_fit <- identical(run_profile, "short_test") || isTRUE(rerun_all_after_safe_xi_fix)
...
s4a_data_file <- function(rep_id,
                          root = root_dir,
                          data_dir_arg = "data_s4a_sparse_counts",
                          data_scenario_id_arg = "S4A_EASY_SPARSE_COUNTS_OBS_T100",
...
s4a_fit_file_unused <- function(rep_id,
                         root = root_dir,
                         output_dir_arg = "output_s4a_easy_sparse_counts_fixed_gamma",
                         scenario_id_arg = "S4A_EASY_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100",
    ...
    file.path(
        ...
        output_dir,
        scenario_id,
        sprintf("fit_S4A_easy_sparse_counts_obs_fixed_gamma_rep%02d.rds", as.integer(rep_id))
    )
}

s4a_summary_file_unused <- function(rep_id,
                             root = root_dir,
                             output_dir_arg = "output_s4a_easy_sparse_counts_fixed_gamma",
                             scenario_id_arg = "S4A_EASY_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100",
                             output_dir = output_dir_arg,
    ...
    file.path(
        ...
        output_dir,
        scenario_id,
        sprintf("summary_S4A_easy_sparse_counts_obs_fixed_gamma_rep%02d.csv", as.integer(rep_id))
    )
}
...
fit_s4a_sparse_counts_obs_fixed_gamma_one_rep <- function(rep_id,
                                              scenario_id = "S4A_EASY_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100",
                                              data_scenario_id = "S4A_EASY_SPARSE_COUNTS_OBS_T100",
                                              data_dir = "data_s4a_sparse_counts",
                                              output_dir = "output_s4a_easy_sparse_counts_fixed_gamma",
    ...
    cat(sprintf("=== Scenario 4A sparse-count fixed-gamma fit: rep %s ===\n", rr))
    ...
    s4a_guard_counts <- get_s4a_numeric_guards()
    fit$diagnostics$s4a_numeric_guards <- s4a_guard_counts
    fit$diagnostics$s4a_beta_guard_count <- s4a_guard_counts$n_beta_guard
    fit$diagnostics$s4a_kappa_guard_count <- s4a_guard_counts$n_kappa_guard
    fit$diagnostics$s4a_lambda_input_guard_count <- s4a_guard_counts$n_lambda_input_guard
    fit$diagnostics$s4a_lambda_output_guard_count <- s4a_guard_counts$n_lambda_output_guard

    ## Relabel the returned S3 fit object as a fixed-gamma S4A fit.  The actual
    ## gamma samples are constant because update_gamma_common_s3() is locally
    ## overridden above.
    ...
    fit$metadata$model <- "S4A sparse-count observation-level dynamic NB-ICAR with fixed gamma"
    ...
    ## The S3 summarizer labels gamma as learned.  For this S4A main analysis,
    ## gamma is deliberately fixed at truth, so relabel the gamma fields before
    ## saving the summary.
    ...
    fit$metadata <- c(
        ...
        list(
            ...
            fit_file_prefix = "fit_S4A_easy_sparse_counts_obs_fixed_gamma_rep",
    ...
    if (isTRUE(save_result)) {
        ...
        fit_file <- file.path(out_dir, paste0("fit_S4A_easy_sparse_counts_obs_fixed_gamma_rep", rr, ".rds"))
        csv_file <- file.path(out_dir, paste0("summary_S4A_easy_sparse_counts_obs_fixed_gamma_rep", rr, ".csv"))
...
fit_s4a_sparse_counts_obs_fixed_gamma_batch <- function(reps = 1:10,
                                            scenario_id = "S4A_EASY_SPARSE_COUNTS_OBS_FIXED_GAMMA_T100",
                                            data_scenario_id = "S4A_EASY_SPARSE_COUNTS_OBS_T100",
                                            data_dir = "data_s4a_sparse_counts",
                                            output_dir = "output_s4a_easy_sparse_counts_fixed_gamma",
    ...
    for (rep_id in reps) {
        ...
        fit <- fit_s4a_sparse_counts_obs_fixed_gamma_one_rep(
            rep_id = rep_id,
            scenario_id = scenario_id,
            data_scenario_id = data_scenario_id,
            data_dir = data_dir,
    ...
    summary_file <- file.path(out_dir, "summary_S4A_easy_sparse_counts_obs_fixed_gamma_all_reps.csv")
...
print_s4a_fit_driver_summary <- function(source_manifest, fit_summary, fit_manifest) {
    cat("\n=== Scenario 4A sparse-count observation-level fixed-gamma fitting summary ===\n")
...
## ---- ensure easier S4A source data -----------------------------------------
## The easier S4A control uses the same observation-level construction as the
## official S4A sparse-count data, but with sparse_beta0_shift = -4.0 instead of
## -4.25.  This lets us check whether the numerical instability seen at -4.25
## emerges as sparsity increases.
ensure_s4a_easy_source_data <- function(reps,
                                        root = root_dir,
                                        data_dir_arg = "data_s4a_sparse_counts",
                                        data_scenario_id_arg = "S4A_EASY_SPARSE_COUNTS_OBS_T100",
                                        generator_file = s4a_data_generator_file,
                                        data_dir = data_dir_arg,
                                        data_scenario_id = data_scenario_id_arg,
                                        generate_if_missing = generate_easy_data_if_missing,
                                        overwrite_existing = overwrite_easy_data_if_existing) {
    files <- vapply(
        reps,
        function(rr) s4a_data_file(
            rr,
            root = root,
            data_dir = data_dir,
            data_scenario_id = data_scenario_id
    ...
    all_present <- all(file.exists(files))
    if (all_present) {
        ## Validate existing files against the easier-S4A constants.  If the
        ## check fails, stop rather than silently fitting the wrong data.
        invisible(lapply(files, check_s4a_source_dataset))
        message("Using existing easier S4A source data: ", file.path(data_dir, data_scenario_id))
        return(invisible(TRUE))
    }

    if (!isTRUE(generate_if_missing)) {
        missing_files <- files[!file.exists(files)]
        stop(
            "Easier S4A source data are missing and generate_easy_data_if_missing is FALSE. Missing: ",
            paste(missing_files, collapse = ", "),
            call. = FALSE
        )
    }

    assert_file_exists_s4a(file.path(root, generator_file), "S4A observation-level data generator")
    source(file.path(root, generator_file))

    message("Generating easier S4A observation-level sparse-count data: ",
            file.path(data_dir, data_scenario_id))

    manifest <- run_s4a_observation_sparse_data_generation(
        root = root,
        s3_script = s3_core_file,
        reps = reps,
        TT_use = TT_use,
        sparse_beta0_shift = expected_sparse_beta0_shift,
        beta0_reference_truth = expected_beta0_reference_truth,
        data_dir = data_dir,
        scenario_id = data_scenario_id,
    ...
    print(check_s4a_observation_data_summary(manifest))
    invisible(TRUE)
}

ensure_s4a_easy_source_data(reps_formal)
...
## ---- S4A numerical guard for xi --------------------------------------------
## In very sparse-count fits, occasional extreme beta/phi proposals can make the
## original Scenario 3 compute_s3_xi() overflow because it clamps only the
## linear predictor and then multiplies by exposure.  The lambda FFBS block then
## sees nonfinite or zero zeta = xi * kappa and stops.  This override computes
## xi on the log scale and clamps log(xi), which is numerically equivalent in the
## posterior region but prevents crashes in impossible extreme states.
s4a_log_xi_lower <- -40
s4a_log_xi_upper <-  40
...
cat(sprintf(
    "Using S4A safe compute_s3_xi(): log(xi) clamped to [%.1f, %.1f].\n",
    s4a_log_xi_lower, s4a_log_xi_upper
))


## ---- S4A sparse-count stabilization guards ---------------------------------
## Formal S4A sparse-count runs can have a weak beta--lambda ridge.  In rep 04,
## the chain entered an impossible numerical state (beta0 < -900 and lambda max
## > 1e8), after which kappa became nonpositive or nonfinite.  These guards
## are deliberately conservative: they reject numerically impossible beta draws
## and sanitize kappa/lambda inputs before FFBS.  The guard counts are saved in
## fit$diagnostics so we can decide later whether the stabilization was rarely
## used or whether S4A itself is a failure-mode regime.
.s4a_guard_env <- new.env(parent = emptyenv())
reset_s4a_numeric_guards <- function() {
    .s4a_guard_env$n_beta_guard <- 0L
    .s4a_guard_env$n_kappa_guard <- 0L
    .s4a_guard_env$n_lambda_input_guard <- 0L
    .s4a_guard_env$n_lambda_output_guard <- 0L
    invisible(TRUE)
}
get_s4a_numeric_guards <- function() {
    list(
        n_beta_guard = .s4a_guard_env$n_beta_guard %||% 0L,
        n_kappa_guard = .s4a_guard_env$n_kappa_guard %||% 0L,
        n_lambda_input_guard = .s4a_guard_env$n_lambda_input_guard %||% 0L,
        n_lambda_output_guard = .s4a_guard_env$n_lambda_output_guard %||% 0L
    )
}
reset_s4a_numeric_guards()

## Bounds are intentionally much wider than the truth scale.  They are not meant
## to regularize ordinary posterior mass; they only stop numerical escape.
...
update_beta <- function(beta_current, ...) {
    ...
    if (isTRUE(bad)) {
        .s4a_guard_env$n_beta_guard <- (.s4a_guard_env$n_beta_guard %||% 0L) + 1L
...
update_kappa <- function(y_coarse, lambda_tilde, xi, r, return_diag = TRUE) {
    ## Recompute directly rather than relying on a possibly crashed original
    ## update.  This is the standard Poisson--Gamma conditional update:
    ## kappa | y, lambda, xi, r ~ Gamma(y + r, rate = xi * lambda + r).
    y <- as.matrix(y_coarse)
    L <- as.matrix(lambda_tilde)
    X <- as.matrix(xi)
    if (!identical(dim(y), dim(L)) || !identical(dim(y), dim(X))) {
        stop("S4A safe update_kappa: y, lambda_tilde, and xi must have the same dimensions.", call. = FALSE)
    }
    r_vec <- as.numeric(r)
    ...
    if (any(guard)) {
        .s4a_guard_env$n_kappa_guard <- (.s4a_guard_env$n_kappa_guard %||% 0L) + sum(guard)
    }
    ...
    if (any(bad_k)) {
        .s4a_guard_env$n_kappa_guard <- (.s4a_guard_env$n_kappa_guard %||% 0L) + sum(bad_k)
        kappa[bad_k] <- 1
    }
    ...
    diag <- list(
        ...
        min_kappa = min(kappa),
        max_kappa = max(kappa),
        n_guarded = .s4a_guard_env$n_kappa_guard %||% 0L
    )
...
ffbs_lambda_all <- function(gamma, y_coarse, xi, kappa, a0, b0, return_diag = TRUE) {
    ...
    if (any(bad_in)) {
        .s4a_guard_env$n_lambda_input_guard <- (.s4a_guard_env$n_lambda_input_guard %||% 0L) + sum(bad_in)
    }
    ...
    if (any(bad_out)) {
        .s4a_guard_env$n_lambda_output_guard <- (.s4a_guard_env$n_lambda_output_guard %||% 0L) + sum(bad_out)
...
cat(sprintf(
    "Using S4A sparse-count stabilization guards: beta0 in [%.1f, %.1f], beta in [%.1f, %.1f], log(xi) in [%.1f, %.1f].\n",
...
## ---- S4A fixed-gamma update override ---------------------------------------
## The Scenario 3 MCMC driver calls update_gamma_common_s3() at every iteration.
## For the clean S4A main analysis, gamma is fixed at truth.  We therefore
## locally override update_gamma_common_s3() with a no-op update that returns the
## current gamma unchanged.  This keeps all other S3 dynamic blocks unchanged
## while preventing the learned-gamma diagnostic mismatch from driving the fit.
use_s4a_fixed_gamma_update <- TRUE

if (isTRUE(use_s4a_fixed_gamma_update)) {
    ...
    cat(sprintf("Using S4A fixed-gamma override: gamma fixed at %.3f.\n", fixed_gamma_value))
}
...
## ---- run Scenario 4A fixed-gamma fit ---------------------------------------
fit_summary <- fit_s4a_sparse_counts_obs_fixed_gamma_batch(
    reps = reps_formal,
    scenario_id = scenario_id,
    data_scenario_id = data_scenario_id,
    data_dir = data_dir,
...
fit_files <- file.path(
    fit_dir_full,
    sprintf("fit_S4A_easy_sparse_counts_obs_fixed_gamma_rep%02d.rds", as.integer(reps_formal))
)
...
saveRDS(
    ...
    file.path(fit_dir_full, "run_info_S4A_easy_sparse_counts_obs_fixed_gamma_T100.rds")
)
...
cat("\nScenario 4A sparse-count observation-level fixed-gamma T100 fitting finished successfully.\n")
