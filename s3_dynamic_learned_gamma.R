...
## Scenario 3 core script for the MSSTNB project.
##
## Scenario 3:
##   Dynamic residual risk model with gamma learned inside the MCMC.
##
## Direct comparison:
##   S2_DYNAMIC_FIXED_GAMMA   : same dynamic data, gamma fixed at truth.
##   S3_DYNAMIC_LEARNED_GAMMA : same dynamic data, gamma updated in MCMC.
##
## Design rule:
##   This file mirrors s2_dynamic_fixed_gamma.R as closely as possible.
##   Functions with the same role use the same names except for s2 -> s3.
##   source_s3_dynamic_learned_gamma() only loads dependencies and checks
##   required objects. It never simulates data and never fits the model.
## ============================================================================
.same_dim_s3 <- function(x, target_dim) {
    d <- dim(x)
    !is.null(d) && length(d) == length(target_dim) && all(as.integer(d) == as.integer(target_dim))
}

.require_file_s3 <- function(path) {
    if (!file.exists(path)) {
...
.source_checked_s3 <- function(path, verbose = TRUE) {
    .require_file_s3(path)
    if (isTRUE(verbose)) {
        message("source: ", path)
    }
...
## -----------------------------------------------------------------------------
source_s3_dynamic_learned_gamma <- function(root = ".", verbose = TRUE) {
    ...
    invisible(lapply(files, .source_checked_s3, verbose = verbose))

    ## Scenario 3 now uses the package-level update_beta(), which supports
    ## lambda_tilde = NULL for Scenario 1 and sampled lambda_tilde for Scenario 3.
    ## We keep local update_phi_s3() and update_r_s3() because they are small
    ## Scenario-3-specific wrappers around the same marginal NB likelihood.
    if (isTRUE(verbose)) {
        message("Scenario 3 cleaned version loaded: using package-level update_beta(lambda_tilde = ...).")
    }
    ...
    needed <- c(
        ...
        "update_beta", "update_tau_phi", "update_kappa", "ffbs_lambda_all",
        "recenter"
    )
...
build_s3_spatial <- function(H_obj = H, B_obj = B_ICAR) {
...
validate_s3_fit_objects <- function(dat, priors, spatial) {
    validate_s3_data(dat)

    n1 <- dat$n1
    if (!is.matrix(spatial$H) || .same_dim_s3(spatial$H, c(n1, n1)) == FALSE) {
        stop("spatial$H must be an n1 by n1 matrix.", call. = FALSE)
    }
    if (!is.matrix(spatial$B_ICAR) ||
        .same_dim_s3(spatial$B_ICAR, c(n1, n1 - 1L)) == FALSE) {
        stop("spatial$B_ICAR must be an n1 by n1 - 1 matrix.", call. = FALSE)
    }
    if (!is.matrix(spatial$BHB) ||
        .same_dim_s3(spatial$BHB, c(n1 - 1L, n1 - 1L)) == FALSE) {
    ...
    if (length(priors$beta_mean) != 2L || length(priors$beta_sd) != 2L) {
        stop("This Scenario 3 script expects exactly two covariates.",
             call. = FALSE)
    }
...
make_s3_file_name <- function(prefix, rep_id, ext = "rds") {
...
compute_s3_xi <- function(e, x1, x2, beta0, beta, phi) {
    ...
    if (any(!is.finite(xi)) || any(xi <= 0)) {
        stop("compute_s3_xi produced nonpositive or nonfinite xi.", call. = FALSE)
    }
    xi
}

compute_s3_loglik_poisson_aug <- function(y_coarse, xi, lambda_tilde, kappa) {
    mu <- pmax(xi * lambda_tilde * kappa, .Machine$double.xmin)
    sum(stats::dpois(y_coarse, lambda = mu, log = TRUE))
}

compute_s3_loglik_nb <- function(y_coarse, e, x1, x2, beta0, beta, phi, r, lambda_tilde) {
    n1 <- ncol(y_coarse)
    out <- 0
    for (j in seq_len(n1)) {
...
compute_u_from_phi_s3 <- function(phi, B) {
    BtB <- crossprod(B)
    rhs <- crossprod(B, phi)
...
## -----------------------------------------------------------------------------
## Scenario 3 local MCMC updates
## -----------------------------------------------------------------------------
## update_beta() is now taken from update_regression_revised.R and supports
## optional lambda_tilde.  The local update_phi_s3() and update_r_s3() below
## are retained as compact Scenario-3-specific wrappers around the same marginal
## NB likelihood, conditional on the current lambda_tilde path.

.nb_loglik_s3 <- function(y, mu, r) {
...
.logpost_phi_u_s3 <- function(u, B, BHB, tau_phi, y_coarse, e, x1, x2,
                              beta0, beta, lambda_tilde, r) {
    phi <- as.numeric(B %*% u)
    ll <- compute_s3_loglik_nb(
        y_coarse = y_coarse,
        e = e,
        x1 = x1,
        x2 = x2,
    ...
    lp <- -0.5 * tau_phi * as.numeric(crossprod(u, BHB %*% u))
    ll + lp
}

update_phi_s3 <- function(u_current, B, BHB, tau_phi, y_coarse, e, x1, x2,
                          beta0, beta, lambda_tilde, r, proposal_sd) {
    proposal_sd <- rep(proposal_sd, length.out = length(u_current))
    u_prop <- u_current + stats::rnorm(length(u_current), 0, proposal_sd)
    lp_cur <- .logpost_phi_u_s3(u_current, B, BHB, tau_phi, y_coarse, e, x1, x2,
                                beta0, beta, lambda_tilde, r)
    lp_prop <- .logpost_phi_u_s3(u_prop, B, BHB, tau_phi, y_coarse, e, x1, x2,
                                 beta0, beta, lambda_tilde, r)
    log_alpha <- lp_prop - lp_cur
    accept <- is.finite(log_alpha) && log(stats::runif(1)) < log_alpha
    u_out <- if (accept) u_prop else u_current
    phi_out <- as.numeric(B %*% u_out)
    list(u = u_out, phi = phi_out, accept = accept, log_alpha = log_alpha)
}

.logpost_rj_s3 <- function(rj, y, mu, priors) {
    if (!is.finite(rj) || rj <= 0) {
        return(-Inf)
    }
    .nb_loglik_s3(y, mu, rj) +
        stats::dgamma(rj, shape = priors$r_shape, rate = priors$r_rate, log = TRUE)
}

update_r_s3 <- function(r_current, y_coarse, e, x1, x2, lambda_tilde,
                        beta0, beta, phi, priors, mh_sd,
                        method = "marginal_nb", return_diag = TRUE) {
    n1 <- ncol(y_coarse)
    r_current <- rep(r_current, length.out = n1)
    mh_sd <- rep(mh_sd, length.out = n1)
    r_out <- r_current
    accept <- rep(FALSE, n1)
    log_alpha <- rep(NA_real_, n1)

    for (j in seq_len(n1)) {
        log_r_cur <- log(r_current[j])
        log_r_prop <- stats::rnorm(1, mean = log_r_cur, sd = mh_sd[j])
        r_prop <- exp(log_r_prop)
        mu_j <- e[, j] * exp(beta0 + beta[1] * x1[, j] + beta[2] * x2[, j] + phi[j]) *
            lambda_tilde[, j]
        lp_cur <- .logpost_rj_s3(r_current[j], y_coarse[, j], mu_j, priors) + log_r_cur
        lp_prop <- .logpost_rj_s3(r_prop, y_coarse[, j], mu_j, priors) + log_r_prop
        log_alpha[j] <- lp_prop - lp_cur
        accept[j] <- is.finite(log_alpha[j]) && log(stats::runif(1)) < log_alpha[j]
        if (accept[j]) {
            r_out[j] <- r_prop
        }
    }

    list(r = r_out, accept = accept, diag = list(log_alpha = log_alpha))
}

## Internal input generator. This intentionally mirrors Scenario 1 so that
## Scenario 3 changes only the dynamic residual risk and the fitting blocks
...
.inv_logit_s3 <- function(z) {
    ifelse(z >= 0, 1 / (1 + exp(-z)), exp(z) / (1 + exp(z)))
}

.log_gamma_transition_target_s3 <- function(gamma_common,
                                            lambda_tilde,
                                            y_coarse,
                                            a0 = 10,
                                            gamma_prior = c(1, 1),
                                            eps = 1e-10) {
    gamma_common <- as.numeric(gamma_common)[1]

    if (!is.finite(gamma_common) || gamma_common <= eps || gamma_common >= 1 - eps) {
        return(-Inf)
    }
    if (length(gamma_prior) != 2L || any(!is.finite(gamma_prior)) || any(gamma_prior <= 0)) {
        stop("gamma_prior must be a length-2 positive finite vector.", call. = FALSE)
    }
    if (length(dim(lambda_tilde)) != 2L || length(dim(y_coarse)) != 2L) {
        stop("lambda_tilde and y_coarse must be matrices.", call. = FALSE)
    }
    if (!identical(dim(lambda_tilde), dim(y_coarse))) {
        stop("lambda_tilde and y_coarse must have the same dimension.", call. = FALSE)
    }

    lambda_tilde <- pmax(lambda_tilde, .Machine$double.xmin)
    TT_use <- nrow(lambda_tilde)
    n1 <- ncol(lambda_tilde)

    logp <- stats::dbeta(gamma_common, gamma_prior[1], gamma_prior[2], log = TRUE)

    ## Conditional transition contribution for the multiplicative beta evolution.
    ## For t >= 2, eta_tj = gamma * lambda_tj / lambda_{t-1,j} must lie in (0,1).
    ## The filtering shape entering the t transition is updated through y_{t-1,j}.
    if (TT_use >= 2L) {
        for (j in seq_len(n1)) {
            a_prev <- as.numeric(a0)
            for (tt in 2:TT_use) {
                a_prev <- gamma_common * a_prev + y_coarse[tt - 1L, j]
                alpha <- gamma_common * a_prev
                beta <- (1 - gamma_common) * a_prev

                eta <- gamma_common * lambda_tilde[tt, j] / lambda_tilde[tt - 1L, j]
                if (!is.finite(eta) || eta <= eps || eta >= 1 - eps ||
                    !is.finite(alpha) || !is.finite(beta) || alpha <= 0 || beta <= 0) {
                    return(-Inf)
                }

                ## Include the Jacobian from eta = gamma * lambda_t / lambda_{t-1}
                ## when viewing the transition density as a density in lambda_t.
                logp <- logp +
                    stats::dbeta(eta, shape1 = alpha, shape2 = beta, log = TRUE) +
                    log(gamma_common) - log(lambda_tilde[tt - 1L, j])
            }
        }
    }
...
update_gamma_common_s3 <- function(gamma_current,
    ...
    ## Random walk is symmetric on the logit scale, so include the
    ## log-Jacobian for the target transformed to the logit scale.
    ...
    log_alpha <- (log_target_proposal + log_jac_proposal) -
        (log_target_current + log_jac_current)

    accept <- is.finite(log_alpha) && log(stats::runif(1L)) < min(0, log_alpha)
...
generate_s3_dynamic_inputs <- function(TT_use,
                                       n1_use,
                                       exposure_min = 300,
                                       exposure_max = 1200,
    ...
    for (nm in c("e", "x1", "x2")) {
        if (any(!is.finite(out[[nm]]))) {
            stop("Non-finite values generated in Scenario 3 input matrix: ", nm, call. = FALSE)
        }
    }
...
## -----------------------------------------------------------------------------
simulate_s3_dynamic_learned_gamma_one <- function(seed = 1L,
                                                TT_use = TT,
                                                n1_use = N1,
                                                ...
                                                H_obj = H,
                                                B_obj = B_ICAR,
                                                scenario_id = "S3_DYNAMIC_LEARNED_GAMMA",
                                                rep_id = NA_integer_,
    ...
    if (isTRUE(use_internal_inputs)) {
        inputs <- generate_s3_dynamic_inputs(
            TT_use = TT_use,
            n1_use = n1_use,
    ...
    if (!.same_dim_s3(e, target_dim) ||
        !.same_dim_s3(x1, target_dim) ||
        !.same_dim_s3(x2, target_dim)) {
        stop(sprintf(
            paste0(
                "Scenario 3 inputs do not have dimensions TT_use x n1_use. ",
    ...
    if (!.same_dim_s3(H_obj, c(n1_use, n1_use)) ||
        !.same_dim_s3(B_obj, c(n1_use, n1_use - 1L))) {
        stop(sprintf(
            paste0(
    ...
    if (any(!is.finite(e)) || any(!is.finite(x1)) || any(!is.finite(x2))) {
        stop("Scenario 3 inputs contain non-finite values before count generation.", call. = FALSE)
    }
    ...
    xi <- compute_s3_xi(e = e, x1 = x1, x2 = x2,
    ...
    for (t in seq_len(TT_use)) {
        for (j in seq_len(n1_use)) {
            ...
            if (bad_rate) {
                stop(sprintf(
                    paste0(
                        "Non-finite, negative, or excessively large Poisson rate in Scenario 3 DGP. ",
    ...
    if (anyNA(y_coarse)) {
        stop("rpois produced NA values in Scenario 3 DGP after rate validation.", call. = FALSE)
    }
    ...
    if (!coherent) {
        stop("Scenario 3 generated fine counts are not coherent with coarse counts.", call. = FALSE)
    }
    ...
    out <- list(
        ...
        data_type = "dynamic_lambda_learned_gamma",
        TT = TT_use,
        n1 = n1_use,
        N1 = n1_use,
        ...
        max_exposure = max(e),
        coherent = coherent,
        input_generator = if (isTRUE(use_internal_inputs)) "generate_s3_dynamic_inputs" else "generate_inputs"
    )
    ...
    validate_s3_data(out)
    out
}

simulate_s3_dynamic_learned_gamma_batch <- function(reps = 1:10,
    ...
    manifest_file <- file.path(out_dir, "manifest_S3_dynamic_learned_gamma.csv")
...
## Scenario 3 fitting
## -----------------------------------------------------------------------------
adapt_sd_s3 <- function(current_sd, n_accept, n_trials,
                        target_rate = 0.30,
                        increase = 1.10,
                        decrease = 0.90,
                        min_sd = 1e-4,
...
initialise_s3_state <- function(dat, settings, priors, spatial, gamma_init = NULL) {
    n1 <- dat$n1
    TT_use <- dat$TT
    ...
    xi <- compute_s3_xi(dat$e, dat$x1, dat$x2, beta0, beta, phi)

    gamma_init <- gamma_init %||% settings$gamma_init %||% dat$gamma_star %||% 0.8
    if (length(gamma_init) == 1L) {
        gamma_init <- rep(gamma_init, n1)
    }
    if (length(gamma_init) != n1 || any(!is.finite(gamma_init)) ||
        any(gamma_init <= 0) || any(gamma_init >= 1)) {
        stop("gamma_init must be scalar or length n1 with entries in (0, 1).",
             call. = FALSE)
    }
    ...
    list(
        ...
        gamma = gamma_init,
        gamma_common = mean(gamma_init),
        xi = xi,
        ess_base = ess_base,
        phi_proposal_sd = rep(settings$phi_proposal_sd_init %||% 0.01, n1 - 1L),
        r_proposal_sd = rep(settings$r_proposal_sd_init %||% 0.50, n1),
        gamma_proposal_sd = settings$gamma_proposal_sd_init %||%
            settings$gamma_rw_sd %||% settings$gamma_mh_sd %||% 0.15
    )
}


run_s3_dynamic_learned_gamma_mcmc <- function(dat,
    ...
    samples <- list(
        ...
        gamma = matrix(NA_real_, nrow = n_stored, ncol = n1),
    ...
    diagnostics <- list(
        ...
        gamma_accept_trace = rep(FALSE, n_iter),
        phi_log_alpha = rep(NA_real_, n_iter),
        r_log_alpha = matrix(NA_real_, nrow = n_iter, ncol = n1),
        gamma_log_alpha = rep(NA_real_, n_iter),
        gamma_common_trace = rep(NA_real_, n_iter),
        gamma_proposal_sd_trace = rep(NA_real_, n_iter),
    ...
    state <- initialise_s3_state(dat, settings, priors, spatial, gamma_init)
    phi_accept_window <- 0L
    r_accept_window <- rep(0L, n1)
    gamma_accept_window <- 0L
    store_idx <- 0L
    ...
    for (iter in seq_len(n_iter)) {
        ...
        state$xi <- compute_s3_xi(dat$e, dat$x1, dat$x2,
        ...
        ## 6. common gamma update conditional on the current lambda path.
        gamma_result <- update_gamma_common_s3(
            gamma_current = state$gamma,
            lambda_tilde = state$lambda_tilde,
            y_coarse = dat$y_coarse,
            a0 = A0_use,
            gamma_prior = gamma_prior,
            proposal_sd = state$gamma_proposal_sd
        )
        state$gamma <- gamma_result$gamma
        state$gamma_common <- gamma_result$gamma_common

        ## 7. lambda FFBS with the current sampled gamma.
        ...
        ## 8. Deterministic re-centering for identified beta0, phi, and lambda.
        rc <- recenter(
            beta0 = state$beta0,
            phi = state$phi,
            lambda_tilde = state$lambda_tilde,
        ...
        state$u <- compute_u_from_phi_s3(state$phi, spatial$B_ICAR)
        state$xi <- compute_s3_xi(dat$e, dat$x1, dat$x2,
                                  state$beta0, state$beta, state$phi)
        ...
        diagnostics$gamma_accept_trace[iter] <- isTRUE(gamma_result$accept)
        ...
        diagnostics$gamma_log_alpha[iter] <- gamma_result$log_alpha %||% NA_real_
        diagnostics$gamma_common_trace[iter] <- state$gamma_common
        diagnostics$gamma_proposal_sd_trace[iter] <- state$gamma_proposal_sd
        ...
        gamma_accept_window <- gamma_accept_window + as.integer(isTRUE(gamma_result$accept))
        ...
        if (iter <= n_burnin && iter %% adapt_interval == 0L) {
            ...
            state$gamma_proposal_sd <- adapt_sd_s3(
                current_sd = state$gamma_proposal_sd,
                n_accept = gamma_accept_window,
                n_trials = adapt_interval,
                target_rate = settings$gamma_target_accept %||% 0.35
            )
            phi_accept_window <- 0L
            r_accept_window <- rep(0L, n1)
            gamma_accept_window <- 0L
        ...
        if (verbose > 0L && iter %% verbose == 0L) {
            ...
            gamma_rate <- mean(diagnostics$gamma_accept_trace[i0:iter])
            ...
            cat(sprintf(
                paste0(
                    ...
                    "gamma=%.3f | phi_acc=%.2f r_acc=%.2f gamma_acc=%.2f beta_rej=%.1f\n"
                ),
                iter, n_iter, elapsed, loglik_nb, state$beta0, state$beta[1],
                state$beta[2], mean(state$r), min(state$lambda_tilde),
                max(state$lambda_tilde), state$gamma_common,
                phi_rate, r_rate, gamma_rate, beta_rej
            ))
        }
    }
    ...
    diagnostics$gamma_accept_rate <- mean(diagnostics$gamma_accept_trace)
    ...
    diagnostics$gamma_proposal_sd_final <- state$gamma_proposal_sd
    ...
    list(
        ...
        metadata = list(
            model = "S3 dynamic residual risk NB-ICAR with learned gamma",
            fixed_lambda = FALSE,
            dynamic_lambda = TRUE,
            fixed_gamma = FALSE,
            learned_gamma = TRUE,
            gamma_initial_value = gamma_init,
            gamma_prior = gamma_prior,
            A0 = A0_use,
            B0 = B0_use,
            updated_blocks = c("beta", "phi", "tau_phi", "r", "kappa", "gamma", "lambda"),
            disabled_blocks = c("delta", "omega"),
...
summarise_s3_lambda_recovery <- function(lambda_draws, lambda_truth) {
...
summarise_s3_dynamic_learned_gamma_fit <- function(fit, dat, scenario_id = NULL, rep_id = NULL) {
    s <- fit$samples
    ...
    gamma_truth <- dat$gamma_star %||% NA_real_
    if (length(gamma_truth) == 1L) gamma_truth <- rep(gamma_truth, dat$n1)
    gamma_truth_mean <- mean(gamma_truth, na.rm = TRUE)

    gamma_draws <- if (!is.null(s$gamma_common)) {
        as.numeric(s$gamma_common)
    } else if (!is.null(s$gamma)) {
        rowMeans(s$gamma, na.rm = TRUE)
    } else {
        rep(NA_real_, length(s$beta0))
    }
    gamma_q <- as.numeric(stats::quantile(gamma_draws, c(0.025, 0.5, 0.975), na.rm = TRUE))

    lambda_truth <- dat$lambda_tilde_ident %||% dat$lambda_tilde
    lambda_rec <- summarise_s3_lambda_recovery(s$lambda_tilde, lambda_truth)
    ...
    cbind(
        data.frame(
            scenario_id = scenario_id %||% dat$scenario_id %||% "S3_DYNAMIC_LEARNED_GAMMA",
            ...
            gamma_truth_mean = gamma_truth_mean,
            gamma_mean = mean(gamma_draws, na.rm = TRUE),
            gamma_sd = stats::sd(gamma_draws, na.rm = TRUE),
            gamma_q025 = gamma_q[1],
            gamma_q50 = gamma_q[2],
            gamma_q975 = gamma_q[3],
            gamma_bias = mean(gamma_draws, na.rm = TRUE) - gamma_truth_mean,
            gamma_covered = as.integer(gamma_q[1] <= gamma_truth_mean && gamma_truth_mean <= gamma_q[3]),
            gamma_accept_rate = fit$diagnostics$gamma_accept_rate %||% NA_real_,
            gamma_proposal_sd_final = fit$diagnostics$gamma_proposal_sd_final %||% NA_real_,
...
fit_s3_dynamic_learned_gamma_one_rep <- function(rep_id,
                                                 scenario_id = "S3_DYNAMIC_LEARNED_GAMMA",
    ...
    cat(sprintf("=== Scenario 3 dynamic learned-gamma fit: rep %s ===\n", rr))
    ...
    if (isTRUE(save_result)) {
        ...
        dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
        fit_file <- file.path(out_dir, paste0("fit_S3_dynamic_learned_gamma_rep", rr, ".rds"))
        csv_file <- file.path(out_dir, paste0("summary_S3_dynamic_learned_gamma_rep", rr, ".csv"))
...
fit_s3_dynamic_learned_gamma_batch <- function(reps = 1:10,
                                               scenario_id = "S3_DYNAMIC_LEARNED_GAMMA",
                                               data_scenario_id = scenario_id,
                                               data_dir = "data_revised",
                                               output_dir = "output_s3_dynamic_learned_gamma",
    ...
    for (rep_id in reps) {
        ...
        fit <- fit_s3_dynamic_learned_gamma_one_rep(
            rep_id = rep_id,
            scenario_id = scenario_id,
            data_scenario_id = data_scenario_id,
            data_dir = data_dir,
    ...
    summary_file <- file.path(out_dir, "summary_S3_dynamic_learned_gamma_all_reps.csv")
...
sanity_check_s3_dynamic_learned_gamma <- function(root = ".",
                                                  rep_id = 1L,
                                                  scenario_id = "S3_DYNAMIC_LEARNED_GAMMA",
                                                  data_scenario_id = scenario_id,
                                                  data_dir = "data_revised",
                                                  output_dir = "output_s3_dynamic_learned_gamma_sanity",
                                                  ...
                                                  verbose = 500L,
                                                  ...) {
    dat_dir <- file.path(root, data_dir, data_scenario_id)
    dat_file <- file.path(dat_dir, paste0("data_rep", sprintf("%02d", as.integer(rep_id)), ".rds"))

    if (!file.exists(dat_file)) {
        if (!isTRUE(simulate_if_missing)) {
            stop("Scenario 3 source data file not found: ", dat_file, call. = FALSE)
        }
        simulate_s3_dynamic_learned_gamma_batch(
            reps = rep_id,
            data_dir = data_dir,
    ...
    fit_s3_dynamic_learned_gamma_one_rep(
        rep_id = rep_id,
        scenario_id = scenario_id,
        data_scenario_id = data_scenario_id,
        data_dir = data_dir,
...
check_s3_dynamic_learned_gamma <- function(fit = NULL, dat = NULL) {
    out <- list()
    if (!is.null(dat)) {
        lambda_truth <- dat$lambda_tilde_ident %||% dat$lambda_tilde
    ...
    if (!is.null(fit)) {
        ...
        out$fit_learned_gamma <- isTRUE(fit$metadata$learned_gamma)
        out$fit_gamma_mean <- if (!is.null(fit$samples$gamma_common)) {
            mean(fit$samples$gamma_common, na.rm = TRUE)
        } else if (!is.null(fit$samples$gamma)) {
            mean(fit$samples$gamma, na.rm = TRUE)
        } else {
            NA_real_
        }
        out$gamma_accept_rate <- fit$diagnostics$gamma_accept_rate %||% NA_real_
        ...
        out$gamma_sampled <- "gamma" %in% fit$metadata$updated_blocks
    }
    out
}
